...
def train_model(model, train_loader, val_loader, device,
                epochs=150, lr=1e-4, patience=20, model_path="model.pth"):
...
# ═══════════════════════════════════════════════════════════
# 4. VMD decomposition of power time series
# ═══════════════════════════════════════════════════════════
power_raw = df[target_cols].values[:, 0].astype(float)
print(f"\n[VMD] Decomposing power series (N={len(power_raw)}) ...")
t0 = time.time()
vmd_decomp = VMDDecomposer(K=4, alpha=2000, tol=1e-7, max_iter=500, seed=SEED)
imfs = vmd_decomp.fit_transform(power_raw)
print(f"  Complete in {time.time() - t0:.1f}s  |  IMFs shape: {imfs.shape}")
print(f"  Centre frequencies (rad/sample): {np.round(vmd_decomp.omega_, 4)}")

residual = power_raw - imfs.sum(axis=1)
print(f"  Residual mean={residual.mean():.4f}  std={residual.std():.4f}  "
      f"max_abs={np.abs(residual).max():.4f}")
...
imfs_scaler.fit(imfs[train_years_mask])
imfs_scaled = imfs_scaler.transform(imfs)

X_seq, y_seq, idx = create_sequences(imfs_scaled, y_scaled, SEQ_LEN)
seq_times = times[idx]
...
print(f"  IMF scaled range: [{imfs_scaled.min():.3f}, {imfs_scaled.max():.3f}]")

# ═══════════════════════════════════════════════════════════
# 6. Train VMD-LSTM Hybrid
# ═══════════════════════════════════════════════════════════
print("\n" + "=" * 60)
print("  VMD-LSTM Hybrid Training")
print("=" * 60)
...
np.savez("vmd_imfs.npz", imfs=imfs, omega=vmd_decomp.omega_, residual=residual)
...
fig.suptitle("VMD-LSTM Hybrid — 24h Wind Power Forecast", fontsize=15, fontweight="bold")
...
fig2.suptitle("VMD Decomposition — Power Time Series (first 2000 hours)",
...
for k in range(4):
    ...
    axes2[k + 1].set_title(f"IMF {k + 1}  (omega_k={vmd_decomp.omega_[k]:.4f})")
